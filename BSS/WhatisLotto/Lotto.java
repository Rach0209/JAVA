...
//		});
		...
		cp.add(new CardPanel(lottoType,listOfChkBox,chBoxAll,rdbManual,
				lblNewLabel, lblWon));
...
//			int index = i;
//
//			cp.get(pageCount).getBtnResultInsts()[index].addActionListener(new ActionListener() {
//				@Override
//				public void actionPerformed(ActionEvent e) {
...
//					gameMoney -= 1000;
//					lblWon.setText(String.valueOf(gameMoney) + "원");
//					cp.get(pageCount).getLblResults()[index].setText((index + 1) + ". 미지정");
//				}
//			});
//		}
//
		...
		btnGameClear.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < cp.get(pageCount).getBtnResultDels().length; i++) {
					...
					gameMoney = 0;
					lblWon.setText(String.valueOf(gameMoney) + "원");
					cp.get(pageCount).lblResults[index].setText((index + 1) + ". 미지정");
				}
			}
