...
public class Main {
	public static void main(String[] args) {
		...
		// 자바에서 날짜 나타내는 것.
		// java.util.Calendar
		//Calendar today = new Calendar(); // => 생성 불가. 개발자가 생성자를 감춰놓음 . public하지않다. // Static이 붙은 아이들 => 객체가 아님.
//		Calendar now = Calendar.getInstance(); // 메소드 getInstance()안에 생성자가 들어가있다./ getInstace()는  Static한 메소드임 -> 클래스 이름으로 사용가능. 나중에더배움~
//		
//		// get(field) field의 int 값(1, 2, 5)을 상수로 이름을 붙여줌.
//		System.out.println("상수값 확인 : " + Calendar.YEAR);
//		
//		System.out.println(now.get(Calendar.YEAR));
//		// 상수값을 정할 때, 1월을 0으로 설정해서 +1을 해준 것.
//		System.out.println(now.get(Calendar.MONTH) + 1); // set으로 값을 변경 할 때도 -1만큼 해줘야 원하는 출력값을 받을 수 있다.
